function imgFileQueueError(file, errorCode, message) {
	try {
 ...
 */
function imgFileQueued(file){
	//addReadyFileInfo(file.id,file.name,"成功加载到上传队列");
}
function imgFileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesQueued > 0) {
...
function imgUploadProgress(file, bytesLoaded) {
	try {
...
function imgUploadComplete(file) {
	try {
...
function imgUploadError(file, errorCode, message) {
...
function addImage(fileName,path) {
	var newImg = document.getElementById("imgDiv");
	var showName = document.getElementById("showImgName");
	var imgSrc = document.getElementById("imgsrc");
	showName.value = fileName;
	imgSrc.value = path;
	newImg.src = ".."+path;
